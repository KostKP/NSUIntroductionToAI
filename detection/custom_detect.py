...
def split_image(img, tile_size=640, overlap=48):
    ...
    img_padded = cv2.copyMakeBorder(img, pad_h//2, pad_h - pad_h//2, 
...
def main():
    parser = argparse.ArgumentParser(description='Process an image and predict bounding boxes.')
    ...
    output_img_name = f"{input_filename}.jpg"
    output_txt_name = f"{input_filename}.txt"
    ...
    for box in filtered_boxes:
        ...
        if xmax_adj > xmin_adj and ymax_adj > ymin_adj:
            final_boxes.append((
                cls,
                xmin_adj,
                ymin_adj,
    ...
    for box in final_boxes:
        ...
        cv2.putText(output_img, label, (int(xmin), int(ymin) - 10), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    
    cv2.imwrite(str(output_dir / output_img_name), output_img)
    
    with open(output_dir / output_txt_name, 'w') as f:
        for box in final_boxes:
            f.write(f"{box[0]} {box[1]} {box[2]} {box[3]} {box[4]} {box[5]:.4f}\n")
    
    print(f"Results saved to {output_dir}/{input_filename}.*")
